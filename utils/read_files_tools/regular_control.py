...
# -*- coding: utf-8 -*-
"""
Regular Control Module

This module provides regular control functionality.
"""
...
def cache_regular(value: str) -> str:
    """
    通过正则的方式，读取缓存中的内容

    解析字符串中的缓存引用语法，并替换为实际的缓存值。
    支持多种数据类型的缓存值转换。

    语法示例：
    - $cache{login_init} - 普通缓存引用
    - $cache{int:user_id} - 指定类型的缓存引用
