...
# -*- coding: utf-8 -*-
"""
Allure Report Data Module

This module provides allure report data functionality.
"""
...
class AllureFileClean:
    ...
    def get_case_count(cls) -> "TestMetrics":
        ...
        成功率计算方式：只将真正通过的用例算作成功（与Allure报告一致）
        ...
        try:
            ...
            if _case_count["total"] > 0:
                # 计算用例成功率：只将真正通过的用例算作成功（与Allure报告一致）
                success_count = _case_count["passed"]
                run_case_data["pass_rate"] = round(success_count / _case_count["total"] * 100, 2)

                # 添加计算方式信息（用于调试）
                run_case_data["calculation_method"] = "passed only (consistent with Allure)"
                run_case_data["success_count"] = success_count

            else:
                # 如果未运行用例，则成功率为 0.0
                run_case_data["pass_rate"] = 0.0
                run_case_data["calculation_method"] = "no cases"
                run_case_data["success_count"] = 0
