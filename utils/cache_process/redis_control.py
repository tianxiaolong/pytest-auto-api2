...
# -*- coding: utf-8 -*-
"""
Redis Control Module

This module provides redis control functionality.
"""
...
class RedisHandler:
    ...
    def __init__(self):
        ...
        self.redis = redis.Redis(
            self.host, port=self.port, password=self.password, decode_responses=True, db=self.database
        )

    def set_string(
        self, name: Text, value, exp_time=None, exp_milliseconds=None, name_not_exist=False, name_exit=False
    ) -> None:
        """
        缓存中写入 str（单个）
        :param name: 缓存名称
