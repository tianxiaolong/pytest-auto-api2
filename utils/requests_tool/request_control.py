...
This module provides request control functionality.
"""

"""
HTTP请求控制模块
提供统一的HTTP请求处理功能，支持多种请求类型和数据格式

主要功能：
- 支持多种请求类型（JSON、表单、文件上传、参数等）
- 自动处理请求头和数据格式转换
- 文件上传和下载功能
- 请求响应时间统计
- Allure测试报告集成
...
from utils.other_tools.allure_data.allure_tools import allure_attach, allure_step, allure_step_no
from utils.other_tools.models import RequestType, ResponseData, TestCase
...
class RequestControl:
    ...
    负责处理各种类型的HTTP请求，包括数据格式转换、文件上传、
    响应处理等功能。支持与测试用例的集成和数据缓存。
    ...
    def __init__(self, yaml_case: Dict) -> None:
        """
        初始化请求控制器

        Args:
            yaml_case: 测试用例数据字典，包含请求的所有配置信息
        """
    ...
    def request_type_for_none(self, headers: Dict, method: Text, **kwargs) -> object:
        ...
        _url = self.__yaml_case.url
        res = requests.request(
            method=method, url=cache_regular(_url), data=None, headers=_headers, verify=False, params=None, **kwargs
        )
        return res
    ...
    def request_type_for_params(self, headers: Dict, method: Text, **kwargs):
        ...
        _headers = self.check_headers_str_null(headers)
        res = requests.request(
            method=method, url=cache_regular(url), headers=_headers, verify=False, data={}, params=None, **kwargs
        )
        return res
    ...
    def request_type_for_data(self, headers: Dict, method: Text, **kwargs):
        ...
        _data, _headers = self.multipart_in_headers(ast.literal_eval(cache_regular(str(data))), headers)
        _url = self.__yaml_case.url
        res = requests.request(
            method=method, url=cache_regular(_url), data=_data, headers=_headers, verify=False, **kwargs
        )
    ...
    @classmethod
    def api_allure_step(
        cls, *, url: Text, headers: Text, method: Text, data: Text, assert_data: Text, res_time: Text, res: Text
    ) -> None:
        """在allure中记录请求数据"""
    ...
    def http_request(self, dependent_switch=True, **kwargs):
        ...
        requests_type_mapping = {
            ...
            "export": self.request_type_for_export,
        }
        ...
        if is_run is True or is_run is None:
            ...
            res = requests_type_mapping.get(self.__yaml_case.requestType)(
                headers=self.__yaml_case.headers, method=self.__yaml_case.method, **kwargs
            )
