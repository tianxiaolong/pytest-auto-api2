...
# -*- coding: utf-8 -*-
"""
Teardown Control Module

This module provides teardown control functionality.
"""
...
from utils.other_tools.models import ParamPrepare, ResponseData, SendRequest, TearDown
from utils.read_files_tools.regular_control import cache_regular, regular, sql_regular
...
class TearDownHandler:
    ...
    def jsonpath_replace_data(cls, replace_key: Text, replace_value: Dict) -> Text:
        ...
        # jsonpath 数据解析
        _new_data = jsonpath_replace(change_data=_change_data, key_name="_teardown_case", data_switch=False)
    ...
    def dependent_self_response(self, teardown_case_data: "ParamPrepare", res: Dict, resp_data: Dict) -> None:
        """
    ...
    def send_request_handler(self, data: "TearDown", resp_data: Dict, request_data: Dict) -> None:
        ...
        for i in _send_request:
            ...
            if i.dependent_type == "response":
                exec(self.dependent_type_response(teardown_case_data=i, resp_data=resp_data))
            # 判断请求中的数据
            elif i.dependent_type == "request":
                self.dependent_type_request(teardown_case_data=i, request_data=request_data)
        ...
        for i in _param_prepare:
            ...
            if i.dependent_type == "self_response":
                self.dependent_self_response(
                    teardown_case_data=i, resp_data=resp_data, res=json.loads(res.response_data)
                )
        ...
        if _teardown_data is not None:
            ...
            for _data in _teardown_data:
                if _data.param_prepare is not None:
                    self.param_prepare_request_handler(data=_data, resp_data=json.loads(_resp_data))
                elif _data.send_request is not None:
                    self.send_request_handler(data=_data, request_data=_request_data, resp_data=json.loads(_resp_data))
        self.teardown_sql()
