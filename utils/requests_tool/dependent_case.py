...
# -*- coding: utf-8 -*-
"""
Dependent Case Module

This module provides dependent case functionality.
"""
...
from utils.other_tools.models import DependentCaseData, DependentData, DependentType, TestCase
...
class DependentCase:
    ...
    def _dependent_type_for_sql(
        self, setup_sql: List, dependence_case_data: "DependentCaseData", jsonpath_dates: Dict
    ) -> None:
        """
    ...
    def dependent_handler(
        self, _jsonpath: Text, set_value: Text, replace_key: Text, jsonpath_dates: Dict, data: Dict, dependent_type: int
    ) -> None:
        """处理数据替换"""
        ...
        if _dependent_type is True:
            ...
            try:
                ...
                raise ValueNotFoundError(
                    ...
                    "如已填写，请检查是否存在yaml缩进问题"
                ) from exc
            except TypeError as exc:
                raise ValueNotFoundError(
                    "dependence_case_data下的所有内容均不能为空！" "请检查相关数据是否填写，如已填写，请检查缩进问题"
                ) from exc
        else:
            return False
